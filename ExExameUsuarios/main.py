...
class Exame(Gtk.Window):
    def __init__(self):
        ...
        usuarios_frm = Gtk.Frame(label="Perfís usuario")
        caixaV = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)
        cHenV = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=2)
        dni_lbl = Gtk.Label(label="DNI")
        self.dni_cbt = Gtk.ComboBoxText()
        self.dni_cbt.set_property("width-request", 200)
        cHenV.add(dni_lbl)
        cHenV.add(self.dni_cbt)
        ...
        cHenV2 = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=2)
        nome_lbl = Gtk.Label(label="Nome")
        self.nome_entry = Gtk.Entry()
        self.nome_entry.set_property("width-request", 200)
        cHenV2.add(nome_lbl)
        cHenV2.add(self.nome_entry)

        caixaV.add(cHenV)
        caixaV.add(cHenV2)
        usuarios_frm.add(caixaV)
        ...
        col2 = Gtk.TreeViewColumn("Descrición", crt, text=1)
        ...
        lblNomePerfil = Gtk.Label(label="Nome perfil")
        ...
        lblDescripcionPerfil = Gtk.Label(label="Descrición")
    ...
    def dni_cbt_changed(self, cbt):
        ...
        self.nome_entry.set_text(nome[0][0])
    ...
    def on_pdf_clicked(self, row):
        dni = self.dni_cbt.get_active_text()
        doc = SimpleDocTemplate("perfisUsuarios.pdf", pagesize=A4)
